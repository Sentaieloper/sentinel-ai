<script lang="ts">
	...
	const tweenedCollateral = tweened(0, { duration: 900, easing: cubicOut });
	const tweenedDebt       = tweened(0, { duration: 900, easing: cubicOut });
	const tweenedHealth     = tweened(0, { duration: 900, easing: cubicOut });
	const tweenedAtRisk     = tweened(0, { duration: 700, easing: cubicOut });
	...
	function computeStats(posData: Position[]) {
		totalCollateral = posData.reduce((s, p) => s + (p.collateral ?? 0), 0);
		totalDebt = posData.reduce((s, p) => s + (p.debt ?? 0), 0);
		const hfs = posData.map((p) => p.healthFactor).filter((h): h is number => typeof h === 'number');
		avgHealth = hfs.length ? hfs.reduce((s, h) => s + h, 0) / hfs.length : 0;
		criticalCount = posData.filter(p => p.riskLevel === 'Critical' || p.riskLevel === 'Danger').length;
	}

	$: tweenedCollateral.set(totalCollateral);
	$: tweenedDebt.set(totalDebt);
...
<div class="dashboard">
	...
	<div class="stats-row">
		<div class="stat-card" style="animation-delay: 0ms">
			...
			<div class="stat-value">${Math.round($tweenedCollateral).toLocaleString()}</div>
		</div>
		<div class="stat-card" style="animation-delay: 80ms">
			<div class="stat-label">TOTAL DEBT</div>
			<div class="stat-value">${Math.round($tweenedDebt).toLocaleString()}</div>
		</div>
		<div class="stat-card" style="animation-delay: 160ms">
			<div class="stat-label">AVG HEALTH FACTOR</div>
			<div class="stat-value" style="color: {gaugeColor($tweenedHealth)}">{$tweenedHealth.toFixed(2)}</div>
		</div>
		<div class="stat-card" style="animation-delay: 240ms">
			<div class="stat-label">AT RISK</div>
			<div class="stat-value" style="color: {$tweenedAtRisk > 0 ? 'var(--critical)' : 'var(--safe)'}">{Math.round($tweenedAtRisk)}</div>
		</div>
	</div>
...
<style>
	...
	.stat-card {
		...
		position: relative;
		overflow: hidden;
		animation: stat-in 520ms cubic-bezier(0.2, 0.9, 0.3, 1) both;
	}
	...
	.position-card {
		...
		position: relative;
		overflow: hidden;
		animation: card-in 480ms cubic-bezier(0.2, 0.9, 0.3, 1) both;
	...
	.position-card:hover::after {
		opacity: 0.8;
		animation: card-sweep 1.6s ease-in-out infinite;
	}
	...
	.positions-grid .position-card:nth-child(1) { animation-delay: 0ms;   }
	.positions-grid .position-card:nth-child(2) { animation-delay: 60ms;  }
	.positions-grid .position-card:nth-child(3) { animation-delay: 120ms; }
	.positions-grid .position-card:nth-child(4) { animation-delay: 180ms; }
	.positions-grid .position-card:nth-child(5) { animation-delay: 240ms; }
	.positions-grid .position-card:nth-child(6) { animation-delay: 300ms; }
	...
	.alert-row {
		...
		gap: 12px;
		padding: 12px 16px;
		animation: alert-slide 500ms cubic-bezier(0.2, 0.9, 0.3, 1) both;
	...
	.alerts-list .alert-row:nth-child(1) { animation-delay: 40ms;  }
	.alerts-list .alert-row:nth-child(2) { animation-delay: 120ms; }
	.alerts-list .alert-row:nth-child(3) { animation-delay: 200ms; }
